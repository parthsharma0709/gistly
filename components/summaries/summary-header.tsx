...
export default function SummaryHeader({ title, createdAt, readingTime }: SummaryHeaderProps) {
  return (
    <div className="flex gap-4 mb-4 justify-between">
      <div className="space-y-6">
        <div className="flex flex-wrap items-center gap-7">
          <Badge className="relative px-4 py-1.5 text-sm font-medium bg-white/80 backdrop-blur-xs rounded-full hover:bg-white/90 transition-all duration-200 shadow-xs hover:shadow-md">
            <Sparkles className="h-4 w-4 mr-1.5 text-rose-500" />
            <div className="text-rose-500">AI Summary</div>
          </Badge>
      ...
      <div>
       <Link href="/dashboard" className=" self-start hover:no-underline">
  <Button
