...
const PricingCard = ({ name, price, description, items, id, paymentLink }: PriceType) => {
  return (
    ...
    >
      <div
        className={cn(
          'relative flex flex-col gap-4 h-full lg:gap-8 z-10 p-8 border-[1px] border-gray-400 rounded-2xl',
      ...
      >
        ...
        {price > 0 ? (
          <MotionDiv variants={listVariant} className="space-y-2 flex justify-center w-full">
            <Link
              href={'https://buy.stripe.com/test_bJecN5gtE3xC5W528TabK00'}
              className={cn(
          ...
          <div className="space-y2 flex justify-center w-full">
            ...
            >
              Create with Free Plan <ArrowRight size={18} />{' '}
            </Link>
          </div>
        )}
      </div>
