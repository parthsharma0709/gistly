...
const PricingCard=({name,price ,description,items,id,paymentLink}:PriceType)=>{
    return (
        <MotionDiv variants={listVariant} whileHover={{scale:1.02}} className="relative w-full max-w-lg  hover:scale-105 hover:transition-all duration-300">
   ...
   <div>
    ...
    <p className="text-base-content/80 mt-2">{description}</p>
      ...
      {price>0 ? (<MotionDiv variants={listVariant} className="flex gap-2">
        <p className="text-5xl tracking-tight font-extrabold ">₹{price}</p>
        <div>
            <p className="text-xs uppercase font-semibold">Rupees</p>
            <p className="text-x">/month</p>
        </div>
      </MotionDiv>):(<div className="font-bold text-3xl">FREE</div>)}
    ...
    {price>0? (  <MotionDiv variants={listVariant} className="space-y-2 flex justify-center w-full">
...
export default function PricingSection(){
    return (
        <MotionSection  variants={containerVariants} initial='hidden' whileInView={'visible'} viewport={{once:true,margin:'-100px'}}
        className="relative overflow-hidden" id="pricing">
            <MotionDiv variants={itemVariants} className="py-12 lg:py-24 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 lg:pt-12">
